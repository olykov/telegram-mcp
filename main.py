...
# Global variable to store the main event loop for thread-safe execution
main_loop = None
...
async def run_in_main_loop(coro):
    """
    Execute a coroutine in the main Telethon event loop from a different thread.
    MCP HTTP server runs in a separate thread, so client calls must be bridged.
    """
    global main_loop
...
async def get_chats(page: int = 1, page_size: int = 20) -> str:
    ...
    """
    try:
        dialogs = await run_in_main_loop(client.get_dialogs())
...
async def get_messages(chat_id: Union[int, str], page: int = 1, page_size: int = 20) -> str:
    ...
    """
    try:
        entity = await run_in_main_loop(client.get_entity(chat_id))
        offset = (page - 1) * page_size
        messages = await run_in_main_loop(client.get_messages(entity, limit=page_size, add_offset=offset))
        if not messages:
...
async def send_message(chat_id: Union[int, str], message: str) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.send_message(entity, message))
...
async def list_contacts() -> str:
    ...
    """
    try:
        result = await run_in_main_loop(client(functions.contacts.GetContactsRequest(hash=0)))
        users = result.users
        if not users:
...
async def search_contacts(query: str) -> str:
    ...
    """
    try:
        result = await run_in_main_loop(client(functions.contacts.SearchRequest(q=query, limit=50)))
        users = result.users
        if not users:
...
async def get_contact_ids() -> str:
    ...
    """
    try:
        result = await run_in_main_loop(client(functions.contacts.GetContactIDsRequest(hash=0)))
        if not result:
    ...
    try:
        ...
        if search_query:
            ...
            if from_date_obj or to_date_obj:
                ...
                messages = await run_in_main_loop( client.get_messages(entity, limit=limit, **params))
...
async def list_topics(
    ...
    try:
        ...
        result = await run_in_main_loop(client(
...
async def list_chats(chat_type: str = None, limit: int = 20) -> str:
    ...
    """
    try:
        dialogs = await run_in_main_loop( client.get_dialogs(limit=limit))
...
async def get_chat(chat_id: Union[int, str]) -> str:
    ...
    try:
        ...
        if hasattr(entity, "title"):
            ...
            try:
                participants_count = (await run_in_main_loop( client.get_participants(entity, limit=0))).total
        ...
        try:
            ...
            dialog = await run_in_main_loop( client.get_dialogs(limit=1, offset_id=0, offset_peer=entity))
            if dialog:
                dialog = dialog[0]
...
async def get_contact_chats(contact_id: Union[int, str]) -> str:
    ...
    try:
        # Get contact info
        contact = await run_in_main_loop( client.get_entity(contact_id))
        ...
        common_chats = []
        try:
            common = await run_in_main_loop( client.get_common_chats(contact))
            for chat in common:
...
async def get_last_interaction(contact_id: Union[int, str]) -> str:
    ...
    try:
        ...
        messages = await run_in_main_loop( client.get_messages(contact, limit=5))
...
async def get_message_context(
    ...
    """
    try:
        chat = await run_in_main_loop( client.get_entity(chat_id))
        # Get messages around the specified message
        messages_before = await run_in_main_loop( client.get_messages(chat, limit=context_size, max_id=message_id))
        central_message = await run_in_main_loop( client.get_messages(chat, ids=message_id))
        ...
        if central_message is not None and not isinstance(central_message, list):
            ...
            central_message = []
        messages_after = await run_in_main_loop(client.get_messages(
        ...
        for msg in all_messages:
            ...
            if msg.reply_to and msg.reply_to.reply_to_msg_id:
                try:
                    replied_msg = await run_in_main_loop( client.get_messages(chat, ids=msg.reply_to.reply_to_msg_id))
                    if replied_msg:
...
async def delete_contact(user_id: Union[int, str]) -> str:
    ...
    """
    try:
        user = await run_in_main_loop( client.get_entity(user_id))
        await run_in_main_loop(client(functions.contacts.DeleteContactsRequest(id=[user])))
...
async def block_user(user_id: Union[int, str]) -> str:
    ...
    try:
        user = await run_in_main_loop( client.get_entity(user_id))
        await run_in_main_loop(client(functions.contacts.BlockRequest(id=user)))
...
async def unblock_user(user_id: Union[int, str]) -> str:
    ...
    try:
        user = await run_in_main_loop( client.get_entity(user_id))
        await run_in_main_loop(client(functions.contacts.UnblockRequest(id=user)))
...
async def get_me() -> str:
    ...
    """
    try:
        me = await run_in_main_loop( client.get_me())
...
async def create_group(title: str, user_ids: List[Union[int, str]]) -> str:
    ...
    try:
        ...
        try:
            ...
            result = await run_in_main_loop(client(functions.messages.CreateChatRequest(users=users, title=title)))
            ...
            if hasattr(result, "chats") and result.chats:
                ...
                dialogs = await run_in_main_loop( client.get_dialogs(limit=5))  # Get recent dialogs
                for dialog in dialogs:
...
async def invite_to_group(group_id: Union[int, str], user_ids: List[Union[int, str]]) -> str:
    ...
    """
    try:
        entity = await run_in_main_loop( client.get_entity(group_id))
        users_to_add = []
...
async def leave_chat(chat_id: Union[int, str]) -> str:
    ...
    try:
        ...
        if isinstance(entity, Channel):
            ...
            try:
                await run_in_main_loop(client(functions.channels.LeaveChannelRequest(channel=entity)))
            ...
            try:
                ...
                me = await run_in_main_loop( client.get_me(input_peer=True))
                await run_in_main_loop(client(
                ...
                try:
                    ...
                    me_full = await run_in_main_loop( client.get_me())
                    await run_in_main_loop(client(
                        functions.messages.DeleteChatUserRequest(
...
async def get_participants(chat_id: Union[int, str]) -> str:
    ...
    """
    try:
        participants = await run_in_main_loop( client.get_participants(chat_id))
        lines = [
...
async def send_file(chat_id: Union[int, str], file_path: str, caption: str = None) -> str:
    ...
    try:
        ...
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.send_file(entity, file_path, caption=caption))
...
async def download_media(chat_id: Union[int, str], message_id: int, file_path: str) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        msg = await run_in_main_loop( client.get_messages(entity, ids=message_id))
        ...
        await run_in_main_loop( client.download_media(msg, file=file_path))
        if not os.path.isfile(file_path):
...
async def set_profile_photo(file_path: str) -> str:
    ...
    try:
        await run_in_main_loop(client(
            functions.photos.UploadProfilePhotoRequest(file=await run_in_main_loop( client.upload_file(file_path)))
        ))
...
async def delete_profile_photo() -> str:
    ...
    """
    try:
        photos = await run_in_main_loop(client(
            functions.photos.GetUserPhotosRequest(user_id="me", offset=0, max_id=0, limit=1)
        ))
        if not photos.photos:
            return "No profile photo to delete."
        await run_in_main_loop(client(functions.photos.DeletePhotosRequest(id=[photos.photos[0].id])))
...
async def get_privacy_settings() -> str:
    ...
    try:
        ...
        try:
            settings = await run_in_main_loop(client(
...
async def import_contacts(contacts: list) -> str:
    ...
    try:
        ...
        result = await run_in_main_loop(client(functions.contacts.ImportContactsRequest(contacts=input_contacts)))
...
async def get_blocked_users() -> str:
    ...
    """
    try:
        result = await run_in_main_loop(client(functions.contacts.GetBlockedRequest(offset=0, limit=100)))
...
async def edit_chat_title(chat_id: Union[int, str], title: str) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        if isinstance(entity, Channel):
            await run_in_main_loop(client(functions.channels.EditTitleRequest(channel=entity, title=title)))
        elif isinstance(entity, Chat):
            await run_in_main_loop(client(functions.messages.EditChatTitleRequest(chat_id=chat_id, title=title)))
        else:
...
async def edit_chat_photo(chat_id: Union[int, str], file_path: str) -> str:
    ...
    try:
        ...
        entity = await run_in_main_loop( client.get_entity(chat_id))
        uploaded_file = await run_in_main_loop( client.upload_file(file_path))
        ...
        if isinstance(entity, Channel):
            ...
            await run_in_main_loop(client(functions.channels.EditPhotoRequest(channel=entity, photo=input_photo)))
        elif isinstance(entity, Chat):
...
async def promote_admin(
    ...
    """
    try:
        chat = await run_in_main_loop( client.get_entity(group_id))
        user = await run_in_main_loop( client.get_entity(user_id))
...
async def get_admins(chat_id: Union[int, str]) -> str:
    ...
    try:
        ...
        participants = await run_in_main_loop( client.get_participants(chat_id, filter=ChannelParticipantsAdmins()))
        lines = [
...
async def get_banned_users(chat_id: Union[int, str]) -> str:
    ...
    try:
        ...
        participants = await run_in_main_loop(client.get_participants(
...
async def get_invite_link(chat_id: Union[int, str]) -> str:
    ...
    try:
        ...
        try:
            ...
            result = await run_in_main_loop(client(functions.messages.ExportChatInviteRequest(peer=entity)))
            return result.link
        except AttributeError:
        ...
        try:
            invite_link = await run_in_main_loop( client.export_chat_invite_link(entity))
            return invite_link
        ...
        try:
            if isinstance(entity, (Chat, Channel)):
                full_chat = await run_in_main_loop(client(functions.messages.GetFullChatRequest(chat_id=entity.id)))
...
async def join_chat_by_link(link: str) -> str:
    ...
    try:
        ...
        try:
            ...
            invite_info = await run_in_main_loop(client(functions.messages.CheckChatInviteRequest(hash=hash_part)))
        ...
        result = await run_in_main_loop(client(functions.messages.ImportChatInviteRequest(hash=hash_part)))
...
async def import_chat_invite(hash: str) -> str:
    ...
    try:
        ...
        try:
            ...
            # Try to check invite info first (will often fail if not a member)
            invite_info = await run_in_main_loop(client(functions.messages.CheckChatInviteRequest(hash=hash)))
            if hasattr(invite_info, "chat") and invite_info.chat:
                # If we got chat info, we're already a member
                chat_title = getattr(invite_info.chat, "title", "Unknown Chat")
                return f"You are already a member of this chat: {chat_title}"
        ...
        # Join the chat using the hash
        try:
            result = await run_in_main_loop(client(functions.messages.ImportChatInviteRequest(hash=hash)))
            if result and hasattr(result, "chats") and result.chats:
                chat_title = getattr(result.chats[0], "title", "Unknown Chat")
                return f"Successfully joined chat: {chat_title}"
            return f"Joined chat via invite hash."
...
async def send_voice(chat_id: Union[int, str], file_path: str) -> str:
    ...
    try:
        ...
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.send_file(entity, file_path, voice_note=True))
...
async def forward_message(
    ...
    """
    try:
        from_entity = await run_in_main_loop( client.get_entity(from_chat_id))
        to_entity = await run_in_main_loop( client.get_entity(to_chat_id))
        await run_in_main_loop( client.forward_messages(to_entity, message_id, from_entity))
...
async def edit_message(chat_id: Union[int, str], message_id: int, new_text: str) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.edit_message(entity, message_id, new_text))
...
async def delete_message(chat_id: Union[int, str], message_id: int) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.delete_messages(entity, message_id))
...
async def pin_message(chat_id: Union[int, str], message_id: int) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.pin_message(entity, message_id))
...
async def unpin_message(chat_id: Union[int, str], message_id: int) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.unpin_message(entity, message_id))
...
async def mark_as_read(chat_id: Union[int, str]) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.send_read_acknowledge(entity))
...
async def reply_to_message(chat_id: Union[int, str], message_id: int, text: str) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.send_message(entity, text, reply_to=message_id))
...
async def search_public_chats(query: str) -> str:
    ...
    """
    try:
        result = await run_in_main_loop(client(functions.contacts.SearchRequest(q=query, limit=20)))
        return json.dumps([format_entity(u) for u in result.users], indent=2)
    except Exception as e:
...
async def search_messages(chat_id: Union[int, str], query: str, limit: int = 20) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        messages = await run_in_main_loop( client.get_messages(entity, limit=limit, search=query))
...
async def resolve_username(username: str) -> str:
    ...
    """
    try:
        result = await run_in_main_loop(client(functions.contacts.ResolveUsernameRequest(username=username)))
        return str(result)
    except Exception as e:
...
async def mute_chat(chat_id: Union[int, str]) -> str:
    ...
    try:
        ...
        peer = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop(client(
        ...
        try:
            ...
            peer = await run_in_main_loop( client.get_input_entity(chat_id))
            await run_in_main_loop(client(
                functions.account.UpdateNotifySettingsRequest(
                    peer=peer,
                    settings={
    ...
    try:
        await run_in_main_loop(client(
            functions.messages.ToggleDialogPinRequest(
                peer=await run_in_main_loop( client.get_entity(chat_id)), pinned=True
            )
        ))
...
async def unarchive_chat(chat_id: Union[int, str]) -> str:
    ...
    try:
        await run_in_main_loop(client(
            functions.messages.ToggleDialogPinRequest(
                peer=await run_in_main_loop( client.get_entity(chat_id)), pinned=False
            )
        ))
...
async def get_sticker_sets() -> str:
    ...
    """
    try:
        result = await run_in_main_loop(client(functions.messages.GetAllStickersRequest(hash=0)))
...
async def send_sticker(chat_id: Union[int, str], file_path: str) -> str:
    ...
    try:
        ...
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.send_file(entity, file_path, force_document=False))
...
async def send_gif(chat_id: Union[int, str], gif_id: int) -> str:
    ...
    try:
        ...
        entity = await run_in_main_loop( client.get_entity(chat_id))
        await run_in_main_loop( client.send_file(entity, gif_id))
...
async def get_bot_info(bot_username: str) -> str:
    ...
    """
    try:
        entity = await run_in_main_loop( client.get_entity(bot_username))
        if not entity:
            return f"Bot with username {bot_username} not found."

        result = await run_in_main_loop(client(functions.users.GetFullUserRequest(id=entity)))
...
async def set_bot_commands(bot_username: str, commands: list) -> str:
    ...
    try:
        ...
        # Get the bot entity
        bot = await run_in_main_loop( client.get_entity(bot_username))
...
async def get_history(chat_id: Union[int, str], limit: int = 100) -> str:
    ...
    try:
        entity = await run_in_main_loop( client.get_entity(chat_id))
        messages = await run_in_main_loop( client.get_messages(entity, limit=limit))
...
async def get_pinned_messages(chat_id: Union[int, str]) -> str:
    ...
    try:
        ...
        try:
            ...
            messages = await run_in_main_loop( client.get_messages(entity, filter=InputMessagesFilterPinned()))
        except (ImportError, AttributeError):
            # Fallback - try without filter and manually filter pinned
            all_messages = await run_in_main_loop( client.get_messages(entity, limit=50))
...
if __name__ == "__main__":
    ...
    async def main():
        """Initialize Telegram client and keep it running."""
        global main_loop

        try:
            # Save the main event loop for use in other threads (MCP server)
            main_loop = asyncio.get_running_loop()

            # Start the Telethon client non-interactively
            print("Starting Telegram client...")
            await run_in_main_loop( client.start())
            print("Telegram client started. Ready to receive requests.")

            # Keep the event loop running and handle client updates
            await run_in_main_loop( client.run_until_disconnected())
        except Exception as e:
    ...
    def start_mcp_server():
        ...
        try:
            print("Running MCP HTTP server on http://0.0.0.0:8000...")
    ...
    # Run Telegram client in main thread with its own event loop
    asyncio.run(main())
